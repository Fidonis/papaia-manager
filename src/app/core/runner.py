"""Detached container that runs `papaia-ctl restore`.

Restore cannot run as a job in this process. `papaia-ctl restore` tears the core
stack down with `docker compose down` before unpacking any archive, and
papaia-manager is a service of that same core project (profile `manager`). A
restore started here would be SIGKILLed the moment teardown removed its own
container -- after the stack is down and before a single byte is restored.
...
# per restore point, and `docker run` refuses a duplicate name outright.
...
@dataclass
class RunnerStatus:
    """Live or final state of a restore runner."""
...
def runner_name(restore_point: str) -> str:
    return f"{RUNNER_NAME_PREFIX}{restore_point}"
...
def build_run_args(
    ...
    *,
    ...
    args: list[str] = [
        ...
        "--detach",
        "--name",
        runner_name(restore_point),
        "--label",
        f"{RUNNER_LABEL_KEY}={RUNNER_LABEL_VALUE}",
        "--label",
        f"de.fidonis.restore-point={restore_point}",
        # The runner outlives the manager but must not outlive its own command:
        # a restore that failed has to stay failed and visible, not be retried
        # by the daemon against a half-restored stack.
        "--restart",
        "no",
    ]
...
async def find_runner() -> RunnerStatus | None:
    """Return the current restore runner -- running or finished -- or None.
    ...
    listing = await _docker(
        ...
        "--all",
        "--filter",
        f"label={RUNNER_LABEL_KEY}={RUNNER_LABEL_VALUE}",
        "--format",
        "{{.ID}}",
    )
    ids = [line.strip() for line in listing.splitlines() if line.strip()]
    if not ids:
        return None
    statuses = [_status_from_inspect(json.loads(await _docker("inspect", cid))) for cid in ids]
...
def _status_from_inspect(payload: Any) -> RunnerStatus:
    if not isinstance(payload, list) or not payload or not isinstance(payload[0], dict):
        raise RunnerError("docker inspect returned an unexpected shape for the restore runner")
    entry = payload[0]
    state = entry.get("State") or {}
    labels = ((entry.get("Config") or {}).get("Labels")) or {}
    name = str(entry.get("Name") or "").lstrip("/")
    restore_point = str(labels.get("de.fidonis.restore-point") or "")
    if not restore_point and name.startswith(RUNNER_NAME_PREFIX):
        restore_point = name[len(RUNNER_NAME_PREFIX) :]
...
async def clear_runner(name: str) -> None:
    """Remove a finished runner so the next restore can start.

    Refuses while it is still running: `docker rm` would need `--force`, and
    killing papaia-ctl mid-restore leaves the stack in exactly the half-restored
    state this whole module exists to avoid.
    """
    status = await find_runner()
    if status is not None and status.name == name and status.is_running:
        raise RunnerError(f"restore runner {name} is still running")
    await _docker("rm", name)


def status_to_dict(status: RunnerStatus | None, log: str = "") -> dict[str, Any]:
